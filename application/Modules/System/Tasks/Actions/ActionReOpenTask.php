...
class ActionReOpenTask
{
    public static function boot($request_data, $urid) {
        //check user permission
        if (denied('re_open_task')) return sendErrorResponse('Forbidden');
        ...
        $data['task_status_id'] = TaskStatus::PENDING;

        \DB::beginTransaction();

        //update task
        $update = $record->update($data);
        if (!$update) return sendErrorResponse('Failed to re-open task.');

        \DB::commit();

        //prepare log data
        $new_record = Tasks::getRecord($record->urid);
        Logs::saveLog('Re-Open Task', 'updated', $new_record, $old_record);

        //send response back to user
        return sendResponse('Task Re Opened Successfully');
    }
